...
let myCard, oppCard, flop, turnriver
...
$("#start").on('click', () => {
    ...
    flop = randomSelect(deck, 3)
    turnriver = combination(deck, 2)
    ...
    $('#flop1').attr('src', `./cards/${flop[0]}.gif`);
    $('#flop2').attr('src', `./cards/${flop[1]}.gif`);
    $('#flop3').attr('src', `./cards/${flop[2]}.gif`);
...
$('#result').on('click', () => {
    ...
    turnriver.forEach((e) => {
        let communityCard = flop.concat(e)
        const hand = new Hand
        ...
        data['hand'].push(myRank)
    })

    console.log(data);

    const winRate = data['win'] / 820 * 100

    $('#myResult').text(`${winRate.toFixed(2)}%`)
    $('#oppResult').text(`${(100 - winRate).toFixed(2)}%`)
})
